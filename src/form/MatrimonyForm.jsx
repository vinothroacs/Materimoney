          ...
          <ul className="space-y-6">
            {steps.map((s, i) => (
              <li key={i} className="flex items-center gap-4">
                ...
                <span
                  className={
                    i === currentStep ? "font-bold text-[#5D4037]" : "opacity-70 text-sm"
                  }
                >
                  {s}
                </span>
          ...
          <div className="flex-1 min-h-[350px]">
            ...
            {currentStep === 0 && (
              <div className="grid md:grid-cols-2 gap-6">
                <input className={input} name="fullName" placeholder="Full Name" onChange={handleChange} value={formData.fullName} />
            ...
            {currentStep === 1 && (
              <div className="grid md:grid-cols-2 gap-6">
                <input className={input} name="education" placeholder="Education" onChange={handleChange} value={formData.education} />
                <input className={input} name="occupation" placeholder="Occupation" onChange={handleChange} value={formData.occupation} />
                <input className={input} name="income" placeholder="Income" onChange={handleChange} value={formData.income} />
              </div>
            )}
            ...
            {currentStep === 2 && (
              <div className="grid md:grid-cols-2 gap-6">
                <input className={input} name="father" placeholder="Father Name" onChange={handleChange} value={formData.father} />
                <input className={input} name="mother" placeholder="Mother Name" onChange={handleChange} value={formData.mother} />
                <input className={input} name="grandfather" placeholder="Grandfather Name" onChange={handleChange} value={formData.grandfather} />
                <input className={input} name="grandmother" placeholder="Grandmother Name" onChange={handleChange} value={formData.grandmother} />
                <input className={input} name="siblings" placeholder="Siblings" type="number" onChange={handleChange} value={formData.siblings} />
              </div>
            )}
            ...
            {currentStep === 3 && (
              <div className="space-y-6">
                <div className="grid md:grid-cols-2 gap-6">
                  <input className={input} name="raasi" placeholder="Raasi" onChange={handleChange} value={formData.raasi} />
                  <input className={input} name="star" placeholder="Star" onChange={handleChange} value={formData.star} />
                  <input className={input} name="dosham" placeholder="Dosham" onChange={handleChange} value={formData.dosham} />
                  <input className={input} name="religion" placeholder="Religion" onChange={handleChange} value={formData.religion} />
                  <input className={input} name="caste" placeholder="Caste" onChange={handleChange} value={formData.caste} />
                </div>
                  ...
                  <label className="w-full h-full flex flex-col items-center justify-center cursor-pointer">
                    {formData.horoscope ? (
                      <>
                        ...
                        <p className="text-xs mt-2 text-center text-[#5D4037]">{formData.horoscope.name}</p>
                      </>
                    ) : (
                      <>
                        <CloudArrowUpIcon className="w-10 h-10 text-[#A67C52]" />
                        <p className="text-xs text-center mt-2 text-[#5D4037]">Upload Horoscope</p>
                      </>
                    )}
            ...
            {currentStep === 4 && (
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <input className={input} name="city" placeholder="City" onChange={handleChange} value={formData.city} />
                <input className={input} name="country" placeholder="Country" onChange={handleChange} value={formData.country} />
              </div>
            )}
            ...
            {currentStep === 5 && (
              <div className="max-w-md p-6 bg-white rounded-xl shadow-md space-y-6 border border-[#EEEEEE]">
                <div className="space-y-2">
                  <label className="block text-sm font-semibold text-[#5D4037]">Account Settings</label>
                ...
                <div className="space-y-2">
                  <p className="text-sm font-semibold text-[#5D4037]">Upload Photo</p>
                  <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-lg cursor-pointer bg-[#FAF6F3] hover:bg-[#EEEEEE] border-[#A67C52]/30 transition-all">
                    ...
                    <p className="text-sm text-[#5D4037]">Click to upload photo</p>
            ...
            {currentStep === 6 && (
              <div className="space-y-2 bg-[#EEEEEE]/50 p-6 rounded-xl border border-[#A67C52]/20 text-[#5D4037]">
                ...
                <p><b>Raasi:</b> {formData.raasi}</p>
                <p><b>Account:</b> {formData.privacy}</p>
              </div>
            )}
          </div>
              ...
              onClick={() => {
                ...
                toast((t) => (
                  <div className="text-center space-y-3">
                    <p className="font-bold text-[#5D4037]">Are you sure to submit?</p>
                    <div className="flex justify-center gap-4 mt-3">
                      ...
                      >Confirm</button>
                      <button onClick={() => toast.dismiss(t.id)} className="px-4 py-2 bg-[#EEEEEE] rounded-lg font-bold">Cancel</button>
                    </div>
                  </div>
                ), { duration: Infinity });
              }}
